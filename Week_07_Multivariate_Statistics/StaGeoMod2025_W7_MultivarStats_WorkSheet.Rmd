...
```{r Step 1.1: Load and Prepare Data}
...
# ============================================================================
# STEP 3: Extract environmental variables 
# ============================================================================
...
pred_cols <- setdiff(
  ...
  c("ID", "BIOME.Name","x","y")          # Columns to exclude ("ID", "BIOME.name","x","y")
)


# ============================================================================
# STEP 4: Check for and remove missing values 
# ============================================================================
...
env_vars_full <-  dat[complete.cases(dat[, pred_cols]),pred_cols]  # Check predictor columns]
head(env_vars_full)
```
...
```{r Step 1.2: Perform PCA}

# ============================================================================
# STEP 1: Perform PCA with scaling 
# ============================================================================

pca_result <- prcomp(env_vars_full, scale. = TRUE)
head(pca_result)
...
var_exp[1] / sum(var_exp)

# ============================================================================
# STEP 3: Create variance table 
# ============================================================================
...
```{r Step 1.3: Determine Number of Axes to Retain}

# ============================================================================
# STEP 1: Scree plot 
# ============================================================================

plot (pca_result, type= "lines", main = "Scree Plot") # Version 1.

screeplot(pca_result)                                 # Version 2.

plot(y = variance_tanle$Variance*100,
     x = 1:20,
     type = "b",
     xlab = "PC",
...
# ============================================================================
# Step 2: Broken stick model function 
# ============================================================================

screeplot(pca_result, bstick = T, type = "lines")
...
```{r Step 1.4: Interpret Variable Loadings}
# Extract loadings

loadings_pca <- pca_result$rotation[,1:3]
...
```{r Step 1.5: Visualize PCA Ordination}
# easy plotting hard visualization

biplot(pca_result,       
       choices = 1:2)  #biplot PCA1 & PCA2 the ugly way

# Extract PC scores

pc_scores <- pca_result$x [,c(1:3)]
head(pc_scores)

dim(dat)           # Check Dimensions of used data frames they need to have the same size
dim(pc_scores)
...
plot(pc_scores[,"PC1"],
     pc_scores[,"PC2"],
     pch = 19,
     col = rainbow(16)[as.factor(dat$BIOME.Name)],
     xlab = "PC-1",
     ylab = "PC-2")

for (BiomeUse in unique(dat$BIOME.Name)) {
  PositionsUse <- (pc_scores[c(dat$BIOME.Name == BiomeUse),c("PC1", "PC2")])
...
legend("topright",
       legend = unique(dat$BIOME.Name),
       col = rainbow(16),         #1:length(unique(dat$BIOME.Name))
       pch = 19,
       cex = 0.5,
       bg = NA)

biplot(pca_result, cex = 0.5)
