...
**TASK:** Fill in the function to create sf objects, its arguments (x, coords, crs), coordinate column names, and the geom for sf objects.
...
listw <- nb2listw(neighbours = nb, style = "W")
...
- Moran I Statistic: 0.2297657886  (fill in the value from your output)
- P-value:  2.2e-16 (fill in the value)
- Conclusion: Is there significant spatial autocorrelation?   YES (Yes/No)
- Type: Is it positive or negative autocorrelation?           Positive
- Implication: What does this mean for your analysis?         The data is slightly clustered.     
...
Because of a significant p value and a positive value of the morans test there is a slight autocorrelation of the residuals.
---
...
library(nlme)
gls_model <- gls(____ ~ Elevation + Slope + SoilClim_PC1 + SoilClim_PC2,
                   data = ____)
...
sPlot.Data$residuals <- ____(gls_model)

# Convert to spatial object
library(sp)
sPlot.Spat <- sPlot.Data
____(sPlot.Spat) <- ~ ____ + ____
...
1. What is the nugget (intercept at distance 0)?
2. What is the approximate range (distance where it plateaus)?
3. What is the sill (plateau value)?
4. Does the variogram suggest spatial autocorrelation?
5. What spatial structure is indicated?
...
exponential_model <- ____(____ ~ Elevation + Slope + SoilClim_PC1 + SoilClim_PC2,
                           random = ~ 1 | ____,  # Random intercept by Country
                           correlation = ____(____ = ~ ____ + ____),
                           data = ____)

# View summary
____(exponential_model)

# Compare to basic GLS
____(exponential_model, gls_model)
```
...
gaussian_model <- ____(____ = exponential_model, 
                        correlation = ____(____ = ~ Longitude + Latitude))
...
# View summary
____(spherical_model)

# Compare to GLS
____(spherical_model, gls_model)
```
...
|-------|-----|-----|-----|--------|-------|
| GLS (no spatial) | ___ | ___ | ___ | ___ | ___ |
| Exponential | ___ | ___ | ___ | ___ | ___ |
| Gaussian | ___ | ___ | ___ | ___ | ___ |
| Spherical | ___ | ___ | ___ | ___ | ___ |
...
spatial_model_tuned <- ____(____ = exponential_model,
...
set.seed(123)
Folds <- createFolds(sPlot.Data$X, k = ___)


# Perform cross-validation
CV <- sapply(1:5, function(i) {
  Train <- ____(spatial_model_tuned, data = sPlot.Data[Folds[[i]], ])
  Test <- ____(Train, sPlot.Data[-Folds[[i]], ])
...
predicted_values <- ____(exponential_model, ____)
...
data_sf$residuals <- ____(exponential_model)
...
**Question:** Has the spatial correlation in residuals been reduced? By how much?
...
|-----------|----------|------------|---------|---------|----------------|
| Intercept | ___ | ___ | ___ | ___ | ___ |
| Elevation | ___ | ___ | ___ | ___ | ___ |
| Slope | ___ | ___ | ___ | ___ | ___ |
| SoilClim_PC1 | ___ | ___ | ___ | ___ | ___ |
| SoilClim_PC2 | ___ | ___ | ___ | ___ | ___ |

## 9.2 Random Effects and Spatial Structure

**Random Effects:**
- Country random intercept SD: ___
- Residual SD: ___

**Spatial Correlation Structure:**
- Type: ___ (Exponential/Gaussian/Spherical)
- Range parameter: ___
- Nugget: ___
...
- RMSE: ___
- MAE: ___
- Reduction in Moran's I: ___ to ___
