...
*The Scatterplot shows and exponential decreasing trend.*
* If you fitted a straight line here, what would it capture well, and what would it miss?
*It would capture the total trend on how the Depth is declining with increasing Sources, but not the exponential relationship, thereby also any predictions according to the total trend, over the borders of the actual data would be wrong.*
* Can you identify parts of the depth range where the relationship changes direction (e.g., rises then falls)?
*No there are no Outliers that are different to the exponential relationship we can observe.*
* How might this motivate the need for flexible smoothers in GAMs?
*A linear function cant describe the relationship observed and would even stronger miss any peaks in the relationship if they would appear. A smoothed curve takes in account the distribution of every point and thereby better describes the actual relationship.*
:::
...
*The deeper it gets the less sources are available in general. But there is an exception around 1000m there are less sources (~30) then at 1400 (~35) afterwards it declines again. AT around 3000 it reaches a plateau of around 0.*
* **Complexity:** What is the reported **edf**, and how would you interpret that value in plain language? 
*The reported edf is 8.5, it estimates the complexity of the created smoothed curved it seems complex but not exesssively.*
...
*The gam check schows patterns of residuals, which tells us that our model is not adequate enough. But we dont have any k-index warning as k-index is higher then 1 and the corresponding p-value is not significant. Nevertheless we should try something else: *
...
*I think the polynomial of 4-6 degrees would go through the points at the end and therby oscillate strongly like the runge type behaviour.*
:::
...
*Because it this way you get the predictions in the sacle of the response.*
* **Uncertainty:** What does the shaded band represent, and where does it widen most? Why might that be?
*The ribbon represents the area in ihich 95%of the point used for that fitted line should lay.*
* **Edge behaviour:** Do you see wider intervals near minimum/maximum depth? How does sampling density play a role?  
*We see a little bit bigger intervals towards the min and max because of the Runges phenomenon but its not really strong.*
...
*AT the areas where the points are most scattered.*
* **Edge stability:** How does the **GAM** behave at shallowest/deepest depths compared with polynomials of degree ≥ 4?  
*GAM actually "oscillates" way stronger then polynomials of the degree 4 as the GAM describes the individual points stronger*
...
*Yes i would still prefer GAM as with increasing degrees of polynomials also the degrees of freedom increase and thereby the fit which is calculated by AIC gets better but also the possibility because of an missunderstnading of the complexity can occur. GAM doesnt get an as low AIC but is a better fit according to showing the complexity of the data. *
...
*The Predictions could be either to high or to low therefor stakeholders could expect to profit more or less of something and thereby take wrong meassures.*
:::
...
*Yes with more K the AIC is lower and therefore better but also the edf is rising which implements probable problems.*
* **k-index check:** Does `gam.check()` flag basis inadequacy (k-index test)? If yes, what new `k` would you try next, and why?  
*both are a little over 1 and therby no warning arises, its higher for K30 thereby i have the assumption that it would further rise for higher K but with the cost of higher edfs as well.*
* **Parsimony vs safety:** Given similar fits, which `k` would you keep for subsequent analyses, and what’s your rationale?  
*K30s AIC is strongly higher i would take these at the cost of higher edf because aslo the value for the k test is better and thereby indicates a safety factor..*
...
*Because edf will increase and thereby the line could not really relate to the points.*
:::
...
*its linear and normal distributed, but maybe heteroscedasticit*
* **k-index:** Does `gam.check()` suggest that the basis size `k` was adequate? If not, what would you try next?
*K index was slightly higher then one we could increase k a bit to be safe.*
* **Concurvity:** Why is concurvity not an issue here, but critical when you have multiple smooth terms?
*because the values of s depth are close to 0 and thereby no concurvity is expected.*
* **Practice:** How would high concurvity affect your interpretation of a GAM with two correlated covariates?
*it would show make it difficult to see which of the covariats effects the response in which way so we could just analyse their effects combined. *
:::
...
*As the edf is higher than 1 the relationship is non linear and because the p value is highly significant, linear models seem to be unsuitable.*
...
*try tomake the station unstructured mybe by looking on them individually.*
:::
...
*Yes they look very parallel.*
* **Uncertainty:** Where are the ribbons widest? Is that due to sparse data or genuine variability? *the ribbons are widdest at the beginning and end, probably because of sparse data.* 
* **Fit adequacy:** Are there systematic zones where one station’s data consistently sits above/below the fitted line?  
*For station 18 we can see a cluster over the band around 1200-1600 Depth and for Station 13 we see a cluster underneath for 900-1500.*
* **Next model:** If shapes differ, how would **by-smooths** change the specification and interpretation?
*It would show diffrent shape profiles for each station.*
:::
...
*the lowest AIC is for m_pois followed closely by m_nb and thenwith a wide gap comes m_gam. So yes they improve AIC. *
* **Dispersion:** What is the Poisson dispersion ratio? If high, how does NB change the picture? 
*We dont see really overdispersion vfor poisson so nb is not needed.*
...
*Poisson and NB take into account counts not random continuous numbers like gam and can thereby improve the models fit.*
